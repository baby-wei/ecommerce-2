...
class ProductManager(models.Manager):
	...
	def get_related(self, instance):
		# for manytomanyfield hv to use "__in" and ".all()"
...
class Product(models.Model):
	...
	"""
	Wrapping 'objects' with a certain function
	Works similar to that declared in views:
	queryset = Products.objects.filter(active=True) |
	queryset = Products.objects.all().active()
	"""
...
def product_post_saved_receiver(sender, instance, created, *args, **kwargs):
	# sender = <e.g. class Product>
	# instance = <e.g. MP3 Player>
	# created = False | True
	product = instance
